...
export default function ProductView({ setCategory, setDescription, className, reportHandler }) {
  ...
  // const prod = filter_products.filter((item) => item.id === id);
  // console.log(prod);
  // const data = (prod[0]);
  // console.log(data);
  ...
  const getSingleProduct = async (url) => {
    ...
    try {
      ...
      if (data) {
        ...
        const { images, offer_price, price, title, review, description, category } = data;
        setData({
          images: images,
  ...
  // console.log(product);
  // const { fields: { images, offer_price, price } } = product;
                ...
                data.images.map((img) => (
                  ...
                  >
                    <img
                      src={img.url}
                      alt=""
                      className={`w-full h-full object-fit ${src !== img.url ? "opacity-50" : ""
                        } `}
                    />
                  </div>
                ))}
